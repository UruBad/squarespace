var global_shoppingBagItems = {};
var global_favoriteItems = {};
var global_tension = false;
   ...
   if (this.checked) {
	...
	(function () {
		if (jQuery().chosen) {
			var config = {
				...
				'.chosen-select-searchless': {disable_search_threshold: 100, width: '100%'},
				'.chosen-select-searchless-deselect': {disable_search_threshold: 100, width: '100%', allow_single_deselect: false}
			};
	...
	jQuery(window).load(function () {
		...
		(function () {
			...
			jQuery('.flexslider.flexslider-thumbnails .flex-direction-nav .custom_next').click(function () {
			    var slider = $('#slider').data('flexslider');
			    var animatingTo = slider.currentSlide + 1;
			    if (!global_tension) {
			        if (animatingTo > 3) {
			            ...
			            slider.flexAnimate(animatingTo);
			            $(".legend").show();
			        }
			    }
			    else {
			        ...
			        else if (animatingTo > 0 && animatingTo < 4) {
			...
			jQuery('.flexslider.flexslider-thumbnails .flex-direction-nav .custom_prev').click(function () {
			    var slider = $('#slider').data('flexslider');
			    var animatingTo = slider.currentSlide - 1;
			    if (!global_tension) {
			        if (animatingTo < 0) {
			        ...
			        if (animatingTo > 0 && animatingTo < 4) {
			            slider.flexAnimate(0);
			            $(".legend").hide();
			        }
			        else if (animatingTo < 0) {
			            slider.flexAnimate(6);
			            $(".legend").show();
			        }
			        else {
			            slider.flexAnimate(animatingTo);
			...
			jQuery('.flexslider.flexslider-thumbnails .slides .tension_map').click(function () {
			    var slider = $('#slider').data('flexslider');
			    if(slider.currentSlide > 0){
			        if (!global_tension) {
			            slider.flexAnimate(slider.currentSlide + 3);
			            $(".smallThumbnails li[data-target='1']").hide();
			            $(".smallThumbnails li[data-target='4']").show();
			            setActiveThumbnail(slider, 4);
			            global_tension = true;
			        }
			        else {
			            slider.flexAnimate(slider.currentSlide - 3);
			            $(".smallThumbnails li[data-target='4']").hide();
			            $(".smallThumbnails li[data-target='1']").show();
			            setActiveThumbnail(slider, 1);
			            global_tension = false;
			        }
			    }
			    return false;
			});

			/*O.R.: change src to data-src. We use data-src for the fast load of the slider */
			/*jQuery('.thumbnailSlider img[data-src]').each(function () {
				var $this = jQuery(this);
	...
	$.ajax({
		url: "/Product/GetShoppingCartAjax",
		method: "post",
		success: GenerateShoppingCartWidget,
		error: function (data) {
			console.log("ERROR in /Product/GetShoppingCartAjax");
			console.log(data);
		}
	});
	...
	$.ajax({
		...
		error: function (data) {
			console.log("ERROR in /Favorites/GetFavoritesAjax");
			console.log(data);
		}
	});
...
function GenerateShoppingCartWidget(data) {
	global_shoppingBagItems = data;
	var html = '<button type="button" class="btn btn-link pull-right">' +
				'	<span aria-hidden="true" data-icon="&#xe006;"></span> ' + data.quantity + ' item' + (data.quantity == 1 ? '' : 's') + ': ' + data.totalString + '<b class="caret"></b>' +
				'</button>';
	if (data.count > 0) {
		html += '<div role="complementary">';
		var i = 0;
		...
		for (i; i < len; ++i) {
			...
			html +=
				...
				'	<img src="' + item.image + '_70x100.jpg" alt="' + item.title + '">' +
				'	<div class="item-info-name-features-price">' +
				'		<h4><a href="/Product?name=' + item.name + '">' + item.title + '</a></h4>' +
				'		<span class="features">' + item.colorTitle + ', ' + item.sizeTitle + '</span><br>' +
				'		<span class="quantity">' + item.quantity + '</span><b>&times;</b><span class="price">' + item.priceString + '</span>' +
				'	</div>' +
				'	<button type="button" class="close" aria-hidden="true" data-url="/Product/DeleteFromCartAjax?code=' + item.code + '&color=' + item.color + '&size=' + item.size + '"><span aria-hidden="true" data-icon="&#xe005;"></span></button>' +
				'</article>';
		}
		html +=
			'<hr>' +
			'<span class="total-price-tag pull-left">Cart subtotal</span><span class="total-price pull-right">' + data.totalString + '</span>' +
			'<div class="clearfix"></div>' +
			'<a href="#" class="btn btn-primary btn-block">View shopping cart</a>' +
			'<a href="#" class="btn btn-default btn-block">Proceed to checkout</a>' +
			'</div>';
	}
	...
	/* remove element from cart after button close click */
	jQuery(".shopping-cart-widget button.close").click(function () {
		var t = $(this);
...
function getShoppingBagItems() {
	var data = global_shoppingBagItems;
	var result = { data: [] };
	if (data && data.items) {
